...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    const user = (
      (await (await fetch("https://randomuser.me/api")).json()) as results
    ).results[0];
    users.push({
      username: user.email,
  ...
  const posts = [
    {
      ...
      content:
        "In this post, we will learn about the basics of Ably and how it can be used to build powerful realtime applications.",
      comments: [
