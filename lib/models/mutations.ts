...
export async function addComment(
  ...
  if (!response.ok) {
    throw new Error(
      `POST /api/comments: ${response.status} ${JSON.stringify(
        await response.json()
      )}`
    );
  }
...
export async function editComment(
  ...
  if (!response.ok) {
    throw new Error(
      `PUT /api/comments/:id: ${response.status} ${JSON.stringify(
        await response.json()
      )}`
    );
  }
...
export async function deleteComment(mutationId: string, id: number) {
  ...
  if (!response.ok) {
    throw new Error(
      `DELETE /api/comments/:id: ${response.status} ${JSON.stringify(
        await response.json()
      )}`
    );
  }
